...
def write_logs(stdout, stderr, common_output_dir):
    with open(f'{common_output_dir}FLIC.log', 'a') as stdout_f:
        stdout_f.write(stdout)

    with open(f'{common_output_dir}FLIC.log', 'a') as stderr_f:
...
def run_external_tool(cmd, common_output_dir):
    ...
    if process.returncode != 0:
        print(f'The tool returned non-zero. See {os.path.abspath(common_output_dir)}/FLIC.log file', file=sys.stderr)
        sys.exit(1)
...
class Logger:
    def __init__(self, common_output_dir):
        ...
        self.log = open(f'{common_output_dir}FLIC.log', 'a', buffering=1)
