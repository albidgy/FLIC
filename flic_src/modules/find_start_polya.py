...
def find_starts_polya(bam_dir, ref_fasta, common_outdir):
    ...
    logging.info('    Convert bam to bedgraph using bedtools')
    ...
    logging.info('    Convert bedgraph to bigwig using BedGraphToBigWig')
    path_to_bigwig = bedgraph2bigwig(path_to_corrected_bedgraph, file_chr_length, peaks_dir)
    logging.info('    Call peaks using CAGEfightR')
